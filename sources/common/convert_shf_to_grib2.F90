...
INTEGER :: INPUT_UNIT=11, IGRIBSAMPLE1, IGRIBSAMPLE, IGRIBOUT, IGRIBCLONE, IERR
CHARACTER(LEN=30) :: INPUT_FILE="input.shf"
...
! Reading data fields of input file
 ...
 DO WHILE (.TRUE.)
   ...
   READ (INPUT_UNIT, IOSTAT=IERR) DATA(IFIELD) % GRIB2_DESCRIPT
...
! For ensemble forecasts only
   ...
   IF (DATA(IFIELD) % GRIB2_DESCRIPT(7) == 4.OR.DATA(IFIELD) % GRIB2_DESCRIPT(7) == 3) THEN ! type of data: Perturbed forecas or Control forecast
     ...
     DATA(IFIELD) % GRIB2_DESCRIPT(31) = NMEM
     DATA(IFIELD) % GRIB2_DESCRIPT(32) = IMEM
...
! Output file name
 ...
 TRIM(MODEL_NAME),"_",DATA(1) % IDATE0(1:4),"_",NDAY_FC,NHOUR_FC,NMIN_FC,".grib2"
