...
!------------------------------------------------------------------------------
PROGRAM PLGRIBFA_MAIN_INPUT_WORK_BIN
...
CHARACTER :: PPF_TYPE*3, DATAFILE_NAME*14, GRIDTYPE*20, SYSTEM_COMMAND*60,&
 CONTROL1*1, CONTROL2*1, CONTROL3*1, TABLE_NAME*100, LINE*100, &
...
'########################################################################################################################'
  ...
  PRINT *,'Discipline, Category, Parameter             ',READ_2D(IMESSAGE)%PARAM_DISCIPL,&
 READ_2D(IMESSAGE)%PARAM_CATEG, READ_2D(IMESSAGE)%PARAM_IND
  PRINT *,'Level type, Level value, Level value scale  ',&
 READ_2D(IMESSAGE)%LEV_TYPE, READ_2D(IMESSAGE)%LEV_TOP(1), READ_2D(IMESSAGE)%LEV_TOP(2)
...
STOP
END PROGRAM PLGRIBFA_MAIN_INPUT_WORK_BIN
!------------------------------------------------------------------------------
