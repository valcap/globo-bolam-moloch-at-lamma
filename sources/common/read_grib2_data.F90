...
! If time unit is hour:
   ...
   print '(i11, i3.2, i3.2, i14, i3.2)', idate(:)
...
!    V wind component
  ...
  if (iii == 26) then ! Huge (1.e17) value of sea ice thikness in GFS data
    do j=1,jana
    do i=1,iana
      if (field2d(i,j,iii) > 1.e2) field2d(i,j,iii)=0.
    enddo
    enddo
  endif
...
! Check (1st instant only) on the number of levels for each atmospheric variable
  ...
  if (iii > 0.and.ist <= 2) then ! Check only for 1st and 2nd instants (analysis an one fc)
    write(*,*) "In subr. read_data_input_grib2 the no. of levels for different variables"
    write(*,*) "is not the same - the no. of levels for each variable is printed below"
...
! Check (1st instant only) on the number of levels for each atmospheric variable
  ...
  if (iii > 0.and.ist == 1) then  ! Check only for 1st instant
    write(*,*) "In subr. READ_DATA_INPUT_GRIB2 the no. of soil levels for different variables"
    write(*,*) "is not the same - the no. of soil levels for each variable is printed below"
