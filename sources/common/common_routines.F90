...
! Subroutines common to various PREBOLAM versions, NESTING and POSTPC (BOLAM)
! and for PREMOLOCH (MOLOCH)
!=======================================================================
...
! Check that output grid coordinates are internal
   ...
   write(*,*) "Point to be interpolated in subr. interp_spline_2d external of input grid: stop"
   print*, "n, i, j, xo(n), xi(1), yo(n), yi(1)", n, i, j, xo(n), xi(1), yo(n), yi(1)
   stop
   endif
...
!=======================================================================
subroutine plotout(a,n,m,nhf)

! Creates a file containing 2D fields for plotting
...
! with an iterative algorithm (nit: no. of steps) - boundary values remain unchanged
 ...
 real snow(nx,ny), topog(nx,ny), topincr(nx,ny), work(nx,ny), workt(nx,ny)

 work = 0.
 topincr = 0.
 if(maxval(snow).lt.1.e-4) then
 print*, "No snow found in redistr_snow"
 return
 endif
...
! print*, "Initial average snow", avsnowi
 ...
 workt=topog
! call plotout(topog,nx,ny,999)
! call plotout(snow,nx,ny,999)
...
!   workt = workt-0.8*topincr
   ...
   work(i,j) = max(snow(i,j) + topincr(i,j)*dincr*snowav, 0.)
...
! print*, "avsnowi/avsnowf", avsnowi/avsnowf
 snow=snow*avsnowi/avsnowf  ! correction to preserve the snow average

! call plotout(workt,nx,ny,999)
! call plotout(snow,nx,ny,999)
...
! np_out             : number of output points
...
!=======================================================================
subroutine frac_soil_ice_point(tsoil, fice, par_b)

! Following to "pochva" soil and snow scheme:
...
real :: tsoil, par_b, t, fice, dfice, par_fb, par_fa
real :: z, ztanh, zsech

 t=alog(tsoil/t0)
 par_fa=-4./alog((T0+t_fice_crit)/t0)
 z=min(max(par_b, 4.), 12.)
 par_fb=2.-(z-4.)/8.
...
!=======================================================================
subroutine snow_lev_def(snow, nlev, lev_snow)

! Definition of snow levels depth (kg/m/m) and number of last snow level
! inside of snow cover following the approach of "Pochva" soil and snow scheme,
! 1-st snow level is snow top.

!   Input variables:
!   snow          : total specific mass of snow cover (kg/m/m)

!   Output variables:
!   nlev          : the last level inside of snow cover (over soil surface)
!   snow_lev      : snow level depth (kg/m/m), depth of the 1-st level equal zero
...
real :: snow, dlev
integer :: nlev, k
real, dimension(1:nlevsnow) :: lev_snow
real, parameter :: valmiss=-9999., dlev_base=10., dlev_min=5.
 ...
 if (snow > 0.) then
   ...
   do while (.true.)
     if (dlev*float(nlevsnow-1) >= snow) exit
     dlev=dlev+dlev_base
   enddo
   lev_snow(1)=0.
   if (snow >= dlev+dlev_min) then
     nlev=int(snow/dlev)+1
     if (snow-dlev*float(nlev-1) >= dlev_min) nlev=nlev+1
     do k=2,nlev
       lev_snow(k)=snow-float(nlev-k)*dlev
     enddo
     nlev=nlev-1
   else
     nlev=1
...
implicit none
integer, dimension(12) :: imonth=(/31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31/)
integer nyrin, nmonin, ndayin, nhouin, nminin,     &
         iday, ihou, imin, nyrc, nmonc, ndayc, nhouc, nminc, ndayr, j, itday, isum
...
1 itday  = 365
 ...
 if (ndayr.gt.itday) then
   ndayr = ndayr - itday
   nyrc  = nyrc + 1
   if (nyrc.eq.100) nyrc = 00    ! millenium bug
 else
   go to 2
 endif
 go to 1
...
!=======================================================================
subroutine interp_spline_1d_work(f,x,n,fe,xe,ne,iv,alf,ex1,ex2)
...
real, dimension(ne) :: fe1, fe4, xe1, xe4
integer :: k, k1, k2, i, n1
real :: fm, fmm, fp, fpp, xm, xmm, xp, xpp, delx, delxp, delxm, delx1, delx2, &
        delxs, delx1s, delx2s, vm, vmm, vp, vpp, spl, flin, grad1
...
! First loop: extrapolation backward
 ...
 grad1=0.
 do k=1,n1
   grad1=grad1+(fe(k)-fe(k+1))/(xe(k)-xe(k+1))
 enddo
 grad1=grad1/float(n1)
