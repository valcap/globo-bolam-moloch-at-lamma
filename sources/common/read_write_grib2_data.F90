...
INTEGER :: ICENTRE_CODE, IMODEL_CODE
...
! Set TemplateNumber
   ...
   IF (FIELD_2D(IMESSAGE) % IPERIOD(4) /= 0) THEN
     CALL GRIB_SET (IGRIBCLONE, 'endStep', FIELD_2D(IMESSAGE) % IPERIOD(2) )
   ENDIF
...
! Forecast range, accumulation period
  ...
  IF (FIELD_2D(IMESSAGE) % IPERIOD(4) /= 0 ) THEN ! Type of statistical processing
    ...
    FIELD_2D(IMESSAGE) % IPERIOD(2)=FIELD_2D(IMESSAGE) % IPERIOD(2)+FIELD_2D(IMESSAGE) % IPERIOD(3)
